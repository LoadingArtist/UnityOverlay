...
namespace UnityStreamlabs
{
    public static class Streamlabs
    {
        ...
        /// Invoked when a donation is sent.
        /// </summary>
        public static event Action<NewFollow> OnNewFollower;
        ...
        private static void HandleSocketMessage (object sender, MessageEventArgs evt)
        {
            try
            {
                ...
                if (data.StartsWith("2[\"event\",{\"type\":\"donation")) // event
                {
                    var eventData = data.Substring(10, data.Length - 11);
                    Debug.Log(eventData);
                    var donationEvent = JsonUtility.FromJson<Donation>(eventData);
                    if (donationEvent.type == Donation.Type)
                    {
                        Debug.Log($"Donation received: From: { donationEvent.message[0].from} Message: { donationEvent.message[0].message} Amount: { donationEvent.message[0].formattedAmount}");
                    }
                }


                if (data.StartsWith("2[\"event\",{\"type\":\"follow")) // event
                {
                    var eventData = data.Substring(10, data.Length - 11);
                    Debug.Log(eventData);
                    var followEvent = JsonUtility.FromJson<NewFollow>(eventData);
                    if (followEvent.type == NewFollow.Type)
                    {
                        unitySyncContext.Send(SafeInokeNewFollow, followEvent);
                    }
                }
            }
            ...
            void SafeInokeNewFollow(object newFollow) => OnNewFollower?.Invoke(newFollow as NewFollow);
        }
